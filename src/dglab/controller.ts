...
import { buildWaveform, normalizeCustomFrames, WaveformPreset } from "./waveforms.js";
...
export interface SetStrengthOptions {
  channel: DgChannelSelector;
  mode: "set" | "increase" | "decrease" | "zero";
  value?: number;
}
...
export class DgLabController {
  private socketServer: DgLabSocketServer | null = null;
  private armState: ArmState | null = null;
  private currentStrength = { A: 0, B: 0 };
  private deviceLimit = { A: 200, B: 200 };
  private lastCommandAt = 0;

  constructor(private readonly config: ControllerConfig) {}

  async createPairing(options: PairingOptions) {
  ...
  arm(options: ArmOptions) {
    ...
    const maxStrengthA = Math.min(
      clampInt(options.maxStrengthA ?? this.config.maxStrengthA, 0, 200),
      this.config.maxStrengthA,
      this.deviceLimit.A
    );
    const maxStrengthB = Math.min(
      clampInt(options.maxStrengthB ?? this.config.maxStrengthB, 0, 200),
      this.config.maxStrengthB,
      this.deviceLimit.B
    );
  ...
  async clearPulse(channel: DgChannelSelector) {
  ...
  async setStrength(options: SetStrengthOptions) {
    const channels = expandChannels(options.channel);
    const isZeroOnly = options.mode === "zero" || (options.mode === "set" && (options.value ?? 0) <= 0);
    if (!isZeroOnly) this.requireArmed();

    const commands: string[] = [];
    const results: Array<{ channel: DgChannel; previous: number; target: number; command: string | null }> = [];
  ...
  async pulse(options: PulseOptions) {
    this.requireArmed();
  ...
  private async ensureServer(options: PairingOptions): Promise<DgLabSocketServer> {
    ...
    server.on("strength-feedback", (feedback) => this.applyStrengthFeedback(feedback as StrengthFeedback));
    await server.start();
    this.socketServer = server;
    return server;
  }

  private resolvePublicWsUrl(override?: string): string {
    if (override) return override;
    if (this.config.publicWsUrl) return this.config.publicWsUrl;
    return `ws://${detectLanIPv4()}:${this.config.port}`;
  }

  private applyStrengthFeedback(feedback: StrengthFeedback): void {
  ...
  private computeTargetStrength(channel: DgChannel, options: SetStrengthOptions): number {
    if (options.mode === "zero") return 0;
    ...
    if (options.mode === "increase") return Math.min(current + value, effectiveLimit);
    if (options.mode === "decrease") return Math.max(current - value, 0);
    return Math.min(value, effectiveLimit);
  }

  private getValidArmState(): ArmState | null {
    if (!this.armState) return null;
    if (Date.now() <= this.armState.armedUntil) return this.armState;
    this.armState = null;
    return null;
  }

  private requireArmed(): ArmState {
    const armed = this.getValidArmState();
    if (!armed) {
  ...
  private sendNow(command: string): void {
    if (!this.socketServer) {
...
export function configFromEnv(env: NodeJS.ProcessEnv = process.env): ControllerConfig {
  return {
...
function expandChannels(channel: DgChannelSelector): DgChannel[] {
